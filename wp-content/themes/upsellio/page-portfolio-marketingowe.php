...
$per_page = 9;
// WordPress rezerwuje "paged" i "page" dla static pages - obsluga przez $_GET bezposrednio.
// Dodatkowo akceptujemy "strona" jako bezpieczny custom param.
$paged_raw = 0;
if (isset($_GET["strona"])) {
    $paged_raw = (int) $_GET["strona"];
} elseif (isset($_GET["page"])) {
    $paged_raw = (int) $_GET["page"];
} elseif (isset($_GET["paged"])) {
    $paged_raw = (int) $_GET["paged"];
} else {
    $paged_raw = max((int) get_query_var("page"), (int) get_query_var("paged"));
}
      ...
      <?php if (!empty($items_page)) : ?>
        ...
        <?php if ($max_pages > 1) : ?>
          <nav class="mp-pager" aria-label="Paginacja portfolio marketingowego">
            ...
            $prev_url = $paged > 2 ? add_query_arg("strona", $paged - 1, $base_url) : $base_url;
            $next_url = add_query_arg("strona", $paged + 1, $base_url);
            ?>
            <?php if ($paged > 1) : ?>
            ...
            <?php for ($i = 1; $i <= $max_pages; $i++) : ?>
              <?php $page_url = $i > 1 ? add_query_arg("strona", $i, $base_url) : $base_url; ?>
              <?php if ($i === $paged) : ?>
